...
namespace TCG.PaymentGateways.Providers.Stripe.Infrastructure
{
    internal static class ParameterBuilder
    {
                ...
                foreach (var property in obj.GetType().GetRuntimeProperties())
                {
                    ...
                    foreach (var attribute in property.GetCustomAttributes<JsonPropertyAttribute>())
                    {
                        if (value is INestedOptions)
                            ...
                            Middleware.RequestStringBuilder.ProcessPlugins(ref requestString, attribute, property, value, obj);
                    }
                }
            }
        ...
        {
            Middleware.RequestStringBuilder.ApplyParameterToRequestString(ref url, argument, value);
        ...
        private static void ApplyNestedObjectProperties(ref string requestString, object nestedObject)
        {
            foreach (var property in nestedObject.GetType().GetRuntimeProperties())
            {
                ...
                foreach (var attribute in property.GetCustomAttributes<JsonPropertyAttribute>())
                    Middleware.RequestStringBuilder.ProcessPlugins(ref requestString, attribute, property, value, nestedObject);
            }
        }
    }
}
